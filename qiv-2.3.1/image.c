...
void qiv_load_image(qiv_image *q) //把串口命令发送放到这个里面看来。。。。need use sleep(float sec) to delay for switching
{
  struct stat statbuf;
  const char * image_name = image_names[ image_idx];
  Imlib_Image * im=NULL;
  int has_alpha=0, rot;
  //added for switching time out
  int switching_timeout;
  ...
  */
  ...
  send232(image_idx);
  sleep(0.1);		//给串口发改变图像的信息，然后等一下再换图像。

  update_image(q, FULL_REDRAW);
  //// below codes is prevent continuous switching, get the prog continue when hear back from MCU
  switching_timeout =0;
  while( 0==(read232()&2) ) //the bit 1 ie2 is receive done signal.
  {
    sleep(0.1);
    if( (++switching_timeout) > 15) exit(1); // time out for 1.5s and exit
  }
