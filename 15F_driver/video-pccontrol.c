...
{
	float
 g[4],vcc; //amplifier's gain for 4 samples and vcc voltage from the controller
...
int record_video(char* filename_prefix, unsigned int number)
{
char command_line[200];

sprintf(command_line, "./capvideo %s__%d.mp4 \n",filename_prefix,number);
system(command_line);
}
...
unsigned int take_video=0, video_step=0;
time_t t = time(NULL);
struct tm tm = *localtime(&t); //to get time string for the video filenames
char filename_by_time[200];
...
while(1){
	...
	if(level1_input == 4){
		...
		if(level2_input == 1) {
			printf("\nDo you need automatically take video?\n (1) Yes\n (2) No\n");
			if( number_input("Choose (1-2)",1,2,1,2) == 1)
				{
				take_video = 1;
				video_step = number_input("Please input how many test cycles between every video taken. (50-10000)",50,10000,1,100);
				} else {
				take_video = 0;
				}
			sprintf(filename_by_time,"%d-%d-%d_%d-%d_%d", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
			//if (take_video ==1) record_video(filename_by_time,1); //pass prefix filename and #
			for(i=0;i<settings.round_num;i++)
				{
				if( take_video ==1 ) if ((i % video_step)==0) record_video(filename_by_time,i);
